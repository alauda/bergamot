...
func (logger Logger) Logf(level Level, message string, args ...interface{}) {
	logger.LogCallf(logger.size, level, message, args...)
}
...
func (logger Logger) generateStructured(message string, fields Fields) (string, []interface{}) {
	...
	return logger.generateStructuredMsg(message, args...)
}

func (logger Logger) generateStructuredMsg(message string, fields ...interface{}) (format string, args []interface{}) {
	var (
		buf   bytes.Buffer
		index = 0
	)
